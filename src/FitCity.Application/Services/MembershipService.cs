...
public class MembershipService : IMembershipService
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        var message = notification.Title.Contains("approved", StringComparison.OrdinalIgnoreCase)
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            planPrice = await _dbContext.GymPlans
                .AsNoTracking()
                .Where(p => p.Id == membershipRequest.GymPlanId.Value)
                .Select(p => p.Price)
                .FirstOrDefaultAsync(cancellationToken);
        }
