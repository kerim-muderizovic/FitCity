...
class _MobileGymListScreenState extends State<MobileGymListScreen> {
  ...
  Future<void> _loadRecommendations() async {
    ...
    try {
      ...
      if (!mounted) return;
      setState(() => _recommendationError = error.toString());
    } finally {
      if (!mounted) return;
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: buildMobileAppBar(context, title: 'Gyms'),
      ...
      body: SafeArea(
        child: Padding(
          ...
          child: ListView(
            children: [
              Text('Gyms in Sarajevo', style: Theme.of(context).textTheme.titleMedium),
              const SizedBox(height: 10),
              const CurrentGymIndicator(),
              const SizedBox(height: 16),
              SectionTitle(title: 'Recommended trainers'),
              ...
              if (_recommendationsLoading)
                ...
                const Text('No trainer recommendations yet.', style: TextStyle(color: AppColors.muted))
              else
                SizedBox(
                  height: 176,
              ...
              SectionTitle(title: 'Recommended gyms'),
              const SizedBox(height: 8),
              if (_recommendationsLoading)
                const SizedBox(height: 120, child: Center(child: CircularProgressIndicator()))
              else if (_recommendationError != null)
                ...
                const Text('No gym recommendations yet.', style: TextStyle(color: AppColors.muted))
              else
                SizedBox(
                  height: 176,
                  child: ListView.separated(
