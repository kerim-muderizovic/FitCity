...
class _MobileGymDetailScreenState extends State<MobileGymDetailScreen> {
  ...
  Future<void> _requestMembership() async {
    ...
    if (_api.session.value == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Please sign in to request membership.')),
      );
      return;
    }
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Membership request: ${request.status}')),
      );
    } catch (error) {
      if (!mounted) {
        return;
  ...
  Future<void> _payMembershipRequest() async {
    ...
    try {
      final response = await _api.payMembershipRequest(
        requestId: request.id,
        paymentMethod: _membershipPaymentMethod,
      );
      await _loadMembershipState(_gym!.id);
      if (!mounted) {
        return;
        ...
        return SafeArea(
          child: Column(
            ...
            children: [
              ...
              const Text('Choose payment method', style: TextStyle(fontWeight: FontWeight.w700)),
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        if (_trainers.isEmpty)
                          const Text('No trainers assigned yet.', style: TextStyle(color: AppColors.muted))
                        else
                          Wrap(
                            spacing: 8,
                            children: _trainers
                        ...
                        if (!isTrainer) ...[
                          if (membership != null) ...[
                            Text(
                              'Member until ${membership.endDateUtc.toLocal()}',
                            ...
                            const Text('Waiting for approval', style: TextStyle(color: AppColors.accentDeep)),
                            const SizedBox(height: 8),
                            ...
                            const Text('Approved - payment required', style: TextStyle(color: AppColors.accentDeep)),
                            const SizedBox(height: 8),
                            AccentButton(
                          ...
                          OutlinedButton(
                            ...
                            child: const Text('View memberships'),
                          ),
                          const SizedBox(height: 8),
                          TextButton(
                            onPressed: () => Navigator.of(context)
                                .push(MaterialPageRoute(builder: (_) => const MobileBookingScreen())),
                            child: const Text('Book a trainer', style: TextStyle(color: AppColors.accentDeep)),
                          ),
                          const SizedBox(height: 4),
                        ],
                        TextButton(
        ...
        child: Column(
          children: [
            ...
            if (onChat != null)
              TextButton(
                onPressed: onChat,
                child: const Text('Chat', style: TextStyle(color: AppColors.accentDeep, fontSize: 12)),
              ),
          ],
        ),
      ),
