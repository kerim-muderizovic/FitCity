...
class _MobileBookingScreenState extends State<MobileBookingScreen> {
  ...
  Future<void> _loadAvailability() async {
    ...
    try {
      ...
      if (error is FitCityApiException && error.statusCode == 404) {
        setState(() {
          _statusMessage = 'Trainer not found.';
          _availability = null;
          _setSelectedSlot(null);
        });
      } else {
  ...
  Future<void> _createBooking() async {
    if (_selectedTrainer == null || _selectedSlot == null) {
      setState(() => _statusMessage = 'Select an available slot first.');
      return;
    }
    try {
            ...
            child: Padding(
              ...
              child: _loading
                  ...
                  : Column(
                      ...
                      children: [
                        Text('Book a trainer', style: Theme.of(context).textTheme.titleMedium),
                        ...
                        const SizedBox(height: 12),
                        AccentButton(
                          label: _selectedSlot == null ? 'Select a slot' : 'Create booking',
  ...
  Widget _buildSlotList() {
    ...
    if (slots.isEmpty) {
      final reason = _availability?.reason;
      final message = reason != null && reason.isNotEmpty ? reason : 'No slots available in the next 14 days.';
    ...
    return ListView.separated(
      ...
      itemBuilder: (context, index) {
        ...
        final label = '${_formatDate(startLocal)} ${_formatTime(startLocal)} - ${_formatTime(endLocal)}';
        return ListTile(
          title: Text(label),
          subtitle: Text(isAvailable ? 'Available' : 'Booked', style: const TextStyle(color: AppColors.muted)),
  ...
  String _trainerLabel(Trainer trainer) {
    ...
    return '${trainer.userName} - ${rate.toStringAsFixed(0)} KM/hr';
  }
