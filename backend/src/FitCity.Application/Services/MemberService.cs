...
public class MemberService : IMemberService
{
    ...
    public async Task<IReadOnlyList<MemberDto>> GetMembersAsync(Guid requesterId, string requesterRole, CancellationToken cancellationToken)
    {
        ...
        if (string.Equals(requesterRole, UserRole.GymAdministrator.ToString(), StringComparison.OrdinalIgnoreCase))
        {
            ...
            var requestIds = _dbContext.MembershipRequests
                ...
                .Select(r => r.UserId);
            var checkInIds = _dbContext.CheckInLogs
                .AsNoTracking()
                .Where(c => c.GymId == gymId)
                .Select(c => c.UserId);
            var memberIds = membershipIds.Concat(requestIds).Concat(checkInIds).Distinct();
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (string.Equals(requesterRole, UserRole.GymAdministrator.ToString(), StringComparison.OrdinalIgnoreCase))
        {
            ...
            _dbContext.MembershipRequests.Add(new MembershipRequest
            {
                Id = Guid.NewGuid(),
                UserId = user.Id,
                GymId = gymId,
            ...
            _dbContext.Notifications.Add(new Notification
            {
                ...
                Message = "Your membership was created by your gym. Complete payment to activate your pass.",
                Category = "membership",
                IsRead = false,
                CreatedAtUtc = now
            });
