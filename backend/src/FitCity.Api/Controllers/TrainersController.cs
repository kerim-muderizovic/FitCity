...
public class TrainersController : ControllerBase
{
    ...
    [HttpGet("me/profile")]
    [Authorize(Roles = "Trainer")]
    public async Task<ActionResult<TrainerDto>> MyProfile(CancellationToken cancellationToken)
    {
        var userId = User.GetUserId();
        var trainer = await _trainerService.GetMyProfileAsync(userId, cancellationToken);
        return trainer is null ? NotFound() : Ok(trainer);
    }
}
