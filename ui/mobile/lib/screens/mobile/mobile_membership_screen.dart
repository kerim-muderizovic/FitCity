...
class _MobileMembershipScreenState extends State<MobileMembershipScreen> {
  ...
  Future<void> _requestMembership() async {
    ...
    try {
      ...
      setState(() => _statusMessage = context.l10n.membershipRequestStatus(response.status));
    } catch (error) {
      setState(() => _statusMessage = mapApiError(context, error));
    }
  }
  ...
  Future<void> _issueQr(String membershipId) async {
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text(context.l10n.membershipQrToken(qr.token))),
      );
    } catch (error) {
      if (!mounted) {
        return;
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: buildMobileAppBar(context, title: context.l10n.membershipListTitle),
          ...
          child: Padding(
            ...
            child: _loading
                ...
                : Column(
                    ...
                    children: [
                    Text(context.l10n.membershipListTitle, style: Theme.of(context).textTheme.titleMedium),
                    const SizedBox(height: 8),
                    Text(context.l10n.membershipSignedInAs(session?.user.email ?? '-'),
                    ...
                    DropdownButtonFormField<String>(
                      ...
                      decoration: InputDecoration(
                        labelText: context.l10n.membershipSelectGym,
                        filled: true,
                      ...
                      OutlinedButton(
                        ...
                        child: Text(context.l10n.membershipViewActivePass),
                      ),
                      const SizedBox(height: 12),
                      AccentButton(
                        label: context.l10n.membershipRequest,
                      ...
                      Text(context.l10n.membershipActiveListTitle, style: Theme.of(context).textTheme.titleMedium),
                      const SizedBox(height: 8),
                      Expanded(
                        child: _memberships.isEmpty
                            ? Text(context.l10n.membershipNoMemberships,
                                style: const TextStyle(color: AppColors.muted))
                            : ListView.separated(
                                ...
                                itemBuilder: (context, index) {
                                  ...
                                  return Container(
                                    ...
                                    child: Column(
                                      crossAxisAlignment: CrossAxisAlignment.start,
                                      children: [
                                        Text(context.l10n.membershipStatusLine(membership.status),
                                            style: const TextStyle(fontWeight: FontWeight.w700)),
                                        const SizedBox(height: 4),
                                        const SizedBox(height: 4),
                                        Text(context.l10n.membershipValidUntil(
                                            AppDateTimeFormat.dateTime(membership.endDateUtc)),
                                            style: const TextStyle(color: AppColors.muted)),
