...
public class QrController : ControllerBase
{
    ...
    public async Task<ActionResult<object>> Scan([FromBody] QrScanRequest request, CancellationToken cancellationToken)
    {
        var userId = User.GetUserId();
        var role = User.GetUserRole();
        try
        {
            var success = await _qrService.ScanAsync(userId, role, request, cancellationToken);
