...
public class MembershipsController : ControllerBase
{
    ...
    public async Task<ActionResult<MembershipRequestDto>> Decide(Guid id, [FromBody] MembershipRequestDecision request, CancellationToken cancellationToken)
    {
        ...
        try
        {
            var response = await _membershipService.DecideRequestAsync(id, request.Approve, adminId, adminRole, cancellationToken);
    ...
    public async Task<ActionResult<MembershipPaymentResponse>> Pay(Guid id, [FromBody] MembershipPaymentRequest request, CancellationToken cancellationToken)
    {
        var userId = User.GetUserId();
        var response = await _membershipService.PayMembershipRequestAsync(id, userId, request, cancellationToken);
        return Ok(response);
    }
